...
namespace Elements.Core
{
        ...
        where T : struct
    {
        ...
        public RawValueList(IList<T> elements)
        ...
        public void AddRange(RawValueList<T> items)
        {
        ...
        public void FastRemoveRange(int index, int count)
        {
            var endIndex = index + count;
            var remainderCount = Count - endIndex;

            if(remainderCount > 0)
                Array.Copy(Elements, endIndex, Elements, index, remainderCount);
        ...
        /// <param name="index">Index to insert the item.</param>
        /// <param name="item">Element to insert.</param>
        public void FastInsert(int index, in T item)
        {
            if (index < Count)
            {
                if (Count == Elements.Length)
        ...
        public struct Enumerator : IEnumerator<T>
        {
            ...
            public Enumerator(RawValueList<T> list)
            {
                index = -1;
                this.list = list;
            }
