...
namespace Elements.Core
{
    ...
    public class RawList<T> : IList<T>, IReadOnlyList<T>
    {
        ...
        RawList(T[] wrappedArray, int count)
        {
            Elements = wrappedArray;
            Count = count;
        }

        public static RawList<T> WrapArray(T[] array, int count)
        {
            return new RawList<T>(array, count);
        }
        ...
        public void AddRange(T[] items, int count)
        {
            ...
            Array.Copy(items, 0, Elements, Count, count);
            Count = neededLength;
        }
